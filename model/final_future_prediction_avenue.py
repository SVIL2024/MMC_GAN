...
class Encoder(torch.nn.Module):
    def __init__(self, t_length = 5, n_channel =3):
        ...
        self.mask_ = Upsample(128, 64)
        self.mask = Mask(128, n_channel, 64)

        self.skip2 = Basic(256, 64) 
        self.skip3 = Basic(512, 64)
    ...
    def forward(self, x):
        ...
        return tensorConv4, self.skip2(tensorConv3.detach()),self.skip3(tensorConv4.detach()), mask
...
class Decoder(torch.nn.Module):
    def __init__(self, t_length=5, n_channel=3):
        ...
        self.moduleConv = Basic(512+64, 512)
        self.moduleUpsample4 = Upsample(512, 256)

        self.moduleDeconv3 = Basic(256+64, 256)
...
class OffsetNet(torch.nn.Module):
    ...
    def forward(self, x, max_offset1=3, max_offset2=3):
        bs, c, w, h = x.size()
...
class convAE(torch.nn.Module):
    def __init__(self, n_channel=3,  t_length=5, memory_size=50, memory_dim=256, beta1=1, beta2=0.25, beta3=1, gamma1=1, gamma2=0.25):
        ...
        self.bkg = nn.Parameter(torch.from_numpy(np.repeat(cv2.imread('./bkg_avenue.jpg', 0).copy()[:, :, None], 3, axis=-1).transpose((2, 0, 1)).astype(np.float32)/127.5 - 1))
        ...
        self.get_err1 = Test_Loss(ks=(6, 18))
