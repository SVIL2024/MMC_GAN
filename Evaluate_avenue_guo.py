...
def MNADEval(model=None):
    ...
    parser.add_argument('--model_dir', type=str,  default='./modelzoo/model_ped2_98.29.pth', help='directory of model')
    ...
    with torch.no_grad():
        for k,(imgs, _) in enumerate(test_batch):
            ...
            dif = ((imgs[:, -3:] - outputs[0]).abs().squeeze(0).cpu().numpy().transpose((1, 2, 0)) * 127.5).astype(
                np.uint8)
    ...
    comb = np.array(anomaly_list1) * hyp_alpha[0] + np.array(anomaly_list2) * hyp_alpha[1] + np.array(anomaly_list3) * \
           hyp_alpha[2]
