...
if __name__ == '__main__':
    ...
    else:#skip_process = False
        img_files = glob.glob(prefix+"*_0.jpg")
        #这个是根据avenue训练出来的模型,得到的offset8
    ...
    #
    #对测试时，得到的score和lable进行可视化 这里做的效果不好
    score = conf_avg(recoder_param[0] * list1 + recoder_param[1]*list2)
    label = m_
    # score = score.squeeze()
    label = label.squeeze()
    ...
    plt.savefig('E:\\DMAD-master\\DMAD-PDM\\modelzoo\\Avenue_05.png', dpi=300, bbox_inches='tight')
    ...
    #
    # # 绘制异常分数（折线图）
    # plt.plot(x, y1, label='anomaly score', linewidth=0.8, color='cornflowerblue')
    #
    # # 绘制异常事件（用阶梯图/柱状图，避免线条重叠）
    # plt.step(x, y2, label='anomalous event', where='post', linewidth=1.2, color='darkorange')
    # # 或用柱状图：plt.bar(x, y2, width=1, color='darkorange', label='anomalous event')
    #
    # # 坐标轴与字体
    # plt.xlabel('frame', fontsize=12)
    # plt.ylabel('anomaly score', fontsize=12)
    # plt.xticks(fontsize=10)
    # plt.yticks(fontsize=10, ticks=[0, 0.5, 1.0])  # 显式设置y轴刻度，确保0和1显示
    #
    # # 图例与范围
    ...
    #
    # # 保存并显示
    # plt.savefig('Avenue_05_improved.png', bbox_inches='tight')
    # plt.show()