...
#跑哪个数据集，切换哪个数据集的方法，上海数据集，用一个ped2的方法试试，看看效果怎么样，应该不会特别好，因为上海数据集的方法应用到了bkg背景信息，跑跑试试
...
def MNADTrain():
    ...
    #根据dataset_type数据集的不同，采用不用的加载方式
    ...
    for epoch in range(args.epochs):
        ...
        if score > early_stop['best_eval_auc']:
            ...
            early_stop['idx'] = 0
            #保存最好结果的模型
            torch.save(model, os.path.join(log_dir, f'bool_ped2.pth'))
        else:
            early_stop['idx'] += 1
