...
def MNADEval(model=None):
    ...
    parser.add_argument('--model_dir', type=str,  default='./modelzoo/new_shanghai_78.51.pth', help='directory of model')
    ...
    if model is None: # if not training, we give a exist model and params path
        model = convAE_me(args.c, 5, args.msize, args.dim)
        try:
            print("传入上海科技预训练模型")
            model.load_state_dict(torch.load(args.model_dir).state_dict(),strict=False)
        except:
            print("进入加载上海科技预训练模型中的except")
