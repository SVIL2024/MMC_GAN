...
from .unet import UNet,UNet_SSPCAB
...
#这是一个包含SSPCAB的Decoder
class Decoder_SSPCAB(torch.nn.Module):
    def __init__(self, t_length=5, n_channel=3, dim=256, r=16):
        super(Decoder_SSPCAB, self).__init__()
        ...
        # 插入 SSPCAB 模块 注意sspcab里面的参数
        self.sspcab = SSPCAB(channels=dim//8)

        ### 两个创新点，布尔大学士的创新点
        self.WavePooling = WaveBlock(dim=dim//8)
        self.PPA = PPA(in_features=dim//8, filters=dim//8)  # in_features：输入通道数，filters：输出通道数
    ...
    def forward(self, x, skip3, skip4):
        ...
        tensorDeconv2 = self.moduleDeconv2(tensorUpsample3)
        #在这里加入了SSPCAB模块
        SSPCAB_input = self.moduleUpsample2(tensorDeconv2)
        SSPCAB_output = self.sspcab(SSPCAB_input)

        ### 两个创新点
        X1 = self.WavePooling(SSPCAB_output)  # torch.Size([32, 192, 27, 27])
        X2 = self.PPA(SSPCAB_output)  # torch.Size([32, 192, 27, 27])
        X1_W = self.sigmod(X1)
        SSPCAB_output = X1_W * X2

        output = self.moduleDeconv1(SSPCAB_output)
        mask = self.mask(SSPCAB_output)

        return output, mask, SSPCAB_input, SSPCAB_output
...
# #这个模型并没有完全契合这个文件中的convAE,主要还是训练ped2和ave的模型
# class convAE_Unet_Sspcab(torch.nn.Module):
#     def __init__(self, n_channel=3, t_length=5, memory_size=50, memory_dim=256, beta1=1, beta2=0.25, beta3=1, gamma1=1,
#                  gamma2=0.25, bkg=None, r=32):
#         print("使用convAE_Unet_Sspcab模型")
#         super(convAE_Unet_Sspcab, self).__init__()
#         self.unet_SSPCAB = UNet_SSPCAB(input_channels=12, output_channel=3)
#
#         self.encoder = Encoder(t_length, 3)
#         self.offset_net = OffsetNet(t_length, 3)
#         self.decoder = Decoder(t_length, 3)
#
#         # self.bkg = nn.Parameter(
#         #     torch.from_numpy(cv2.imread('./bkg_ped2.jpg').copy().transpose((2, 0, 1)).astype(np.float32) / 127.5 - 1))
#         #对上海科技数据集背景进行定义
#         if bkg is None:
...
#         # 传入四张图片：要
#         grid1, grid2, offset1, offset2 = self.offset_net(x)  # 这四个都是[1,256,256,2]
#
#         z_e, skip3, skip4 = self.encoder(x)
#         quantized_inputs, commit_loss, vq_loss = self.vq_layer(z_e, test)
#         z_qq,mask = self.decoder(quantized_inputs, skip3, skip4)
#
#         z_q,att,SSPCAB_input , SSPCAB_output = self.unet_SSPCAB(x)
#
#
#         # 要
...
#
#         return z_q_t, z_q_, z_q, z_e, commit_loss, vq_loss, offset1, offset2, mask, mask1, mask2, SSPCAB_input, SSPCAB_output,att
#
#
#     def loss_function(self, x, recon_x, z_q_, z_q, z_e, commit_loss, vq_loss, offset1=None, offset2=None, mask=None,
#                       mask1=None, mask2=None,SSPCAB_input=None, SSPCAB_output=None,att = None,compute_err=False):
#         #下面这两个损失并没有参与训练
...
#         else:
#             self.mse3 = F.mse_loss(SSPCAB_input, SSPCAB_output)
#             self.mse2 = F.mse_loss(recon_x, x)
#             self.mse1 = F.mse_loss(z_q_, x)
#             loss_sparsity = (
#                 torch.mean(torch.sum(-att * torch.log(att + 1e-12), dim=1))
#             )
...
#             self.smooth_loss = self.beta[2] * (
#                         self.loss_smooth1(offset1.permute(0, 3, 1, 2)) + self.loss_smooth2(offset2.permute(0, 3, 1, 2)))
#
#             return self.mse1 + self.mse2 + self.mse3 + self.grad_loss + 0.002*loss_sparsity + \
#                 self.gamma[1] * (self.offset_loss1 + self.offset_loss2 + self.smooth_loss) + \
#                 F.mse_loss(self.bkg.repeat(x.size(0), 1, 1, 1), x)
#
#     def latest_losses(self):
#         return {'mse1': self.mse1, 'mse2': self.mse2, 'sspcab': self.mse3, 'vq': self.vq_loss, 'commitment': self.commit_loss,
#                 'offset1': self.offset_loss1, 'offset2': self.offset_loss2, 'err1_': self.err1_, 'err2_': self.err2_,
#                 'err1': self.err1, 'err2': self.err2, 'grad': self.grad_loss, 'smooth': self.smooth_loss}
...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, idx, test=False):
        #将背景融合到原始图片
        x = torch.cat([x,self.bkg[idx].detach()],dim=1) #这里x的形状从torch.Size([1, 12, 256, 256]) 变为 torch.Size([1, 15, 256, 256])

        print("加入背景后x的形状", x.shape)
        #相当与生成位置坐标信息（运动信息）
        ...
        #类似于记忆模块：根据z_e选出相似的向量
        quantized_inputs, commit_loss, vq_loss = self.vq_layer(z_e, test)
        #mask里面就是0和1用来显示重要区域
        z_q, mask = self.decoder(quantized_inputs,skip2, skip3)
        # print("shanghai训练中，传入记忆模块z_e  quantized_inputs 的形状是：", z_e.shape, quantized_inputs.shape)
...
class convAE_me(torch.nn.Module):
    def __init__(self, n_channel=3,  t_length=5, memory_size=50, memory_dim=256, beta1=1, beta2=0.25, beta3=1, gamma1=1, gamma2=0.25, bkg=None, r=32):
        super(convAE_me, self).__init__()
        print("成功进入convAE_me类")
        self.encoder = Encoder(t_length, 3, memory_dim, r)
        self.offset_net = OffsetNet(t_length, 3)

        #在这里加上一个记忆模块：这个记忆模块里面的参数要定义好，要和编码器和解码器相关
        #这个记忆模块有两个返回值
        self.mem_module1 = MemModule(mem_dim=2000, fea_dim=512, shrink_thres=0.0025)

        #这里的decoder是我修改带有sspcab的encoder，这里的decoder有四个返回值呢，下面不要忘记接收
        self.decoder = Decoder_SSPCAB(t_length, 3,memory_dim, r)
    ...
    def forward(self, x, idx, test=False):
        ...
        #这个z_e是通过编码器得到的特征:形状是torch.Size([1, 512, 32, 32])
        z_e, skip2, skip3 = self.encoder(x)

        #*****************************************
        #类似于记忆模块：根据z_e选出相似的向量，下面这一行代码没有用了，看着删除就行了

        quantized_inputs, commit_loss, vq_loss = self.vq_layer(z_e, test)

        #将上面这个代码，换成记忆模块的相关代码，不要忘记特征对齐
        quantized_inputs = self.mem_module1(z_e)
        #想办法把这个x4里面的att属性返回出去，方便下面做熵损失
        att = quantized_inputs['att']
        quantized_inputs = quantized_inputs['output']

        #******************************************
        # print("shanghai训练中，传入记忆模块z_e  quantized_inputs 的形状是：",z_e.shaper,quantized_inputs.shape)


        #mask里面就是0和1用来显示重要区域 :这里不要忘记加上一个损失函数，用来训练SSPCAB模块
        z_q, mask , SSPCAB_input , SSPCAB_output = self.decoder(quantized_inputs,skip2, skip3)
        ...
        return z_q_t, z_q_, z_q, z_e, commit_loss, vq_loss, offset1, offset2,SSPCAB_input, SSPCAB_output,att, mask, mask1, mask2

    def loss_function(self, x, idx, recon_x, z_q_, z_q, z_e, commit_loss, vq_loss, offset1=None, offset2=None, SSPCAB_input=None, SSPCAB_output=None,att = None, mask=None, mask1=None, mask2=None, compute_err=False,):
        weit = 1+100*torch.abs(F.avg_pool2d((x-self.bkg[idx].detach()).abs()**0.5, kernel_size=15,stride=1, padding=7))
        self.vq_loss = vq_loss
        self.commit_loss = commit_loss

        if compute_err:#这是测试时候的标准
            self.mse2_ = F.mse_loss(recon_x, x, reduction='none')
            self.err1 = self.get_err((offset1 ** 2).sum(-1)**0.5)
            self.err2_ = self.get_err_(self.mse2_)
            return self.mse2_.mean(1)
            ...
            self.grad_loss = self.beta[0]*((self.loss_grad(x, recon_x)*weit).sum()/weit.sum() + (self.loss_grad(x, z_q_)*weit).sum()/weit.sum())
            #这是训练记忆模块的熵损失
            self.loss_sparsity = torch.mean(torch.sum(-att * torch.log(att + 1e-12), dim=1))
            ...
            return  self.mse1+self.mse2+self.grad_loss + self.mse3 + 0.002* self.loss_sparsity + \
                    self.gamma[0]*(self.vq_loss + self.commit_loss) + \
                    self.gamma[1]*(self.offset_loss1+self.offset_loss2 + self.smooth_loss) +\
                    self.mask_loss

    def latest_losses(self):
        return { 'mse1': self.mse1, 'mse2': self.mse2,  'mse3': self.mse3, 'loss_sparsity': self.loss_sparsity, 'vq': self.vq_loss, 'commitment': self.commit_loss, 'mask':self.mask_loss, 'offset1':self.offset_loss1, 'offset2':self.offset_loss2, 'err1_':self.err1_, 'err2_':self.err2_, 'err1':self.err1, 'err2':self.err2, 'grad':self.grad_loss, 'smooth':self.smooth_loss}
