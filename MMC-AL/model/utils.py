...
class DataLoader(data.Dataset):
    def __init__(self, video_folder, transform, resize_height, resize_width, time_step=4, num_pred=1, c=1 ,dataset_type = "ped2" ):
        self.c = c
        ...
        print("目前使用的是数据集：",self.dataset_type)
    ...
    #这个地方针对ped2和avene数据集做了一些调整，因为他们数据集的数量不一样。如果测试上海可以数据集的话，可以在调整一下
    def __getitem__(self, index):
        ...
        if self.dataset_type == "ped2":
            ...
            frame_name = int(self.samples[index].split(SEP)[-1].split('.')[-2]) - 1
        batch = []
