...
class UNet_SSPCAB(nn.Module):
    def __init__(self, input_channels, output_channel=3):
        ...
        ### 两个创新点，布尔大学士的创新点
        self.WavePooling = WaveBlock(dim=64)
        self.PPA = PPA(in_features=64, filters=64)  # in_features：输入通道数，filters：输出通道数
    ...
    def forward(self, x):
        ...
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        # print("传入记忆模块的特征x4的形状是：",x4.shape)
        ...
        SSPCAB_output = self.sspcab(SSPCAB_input) #torch.Size([8, 64, 256, 256])
